...
class YOLOv8Detector:
    def __init__(self, 
                 ...
                 pub_topic1: str,
                 pub_topic2: str, 
                 single_cls: Union[str, None],
                 imgsz: Union[Tuple[int, int], None] = (480, 640), ## h,w
                 conf_thresh: float = 0.5, 
                 iou_thresh: float = 0.45,
                 done_warmup: bool = False,
        ):
        ...
        self.model = AutoBackend(model, device=select_device(self.device),
                                 dnn=True, data=data, fp16=False, fuse=True)
    ...
    def callback(self, stream):
        ...
        preds = ops.non_max_suppression(
            ...
            self.conf,
            self.iou,
            classes=self.single_cls,    
            agnostic=False,
            max_det=100,
            nc=len(self.model.names), ## number of classes
        )
...
if __name__ == '__main__':
    ...
    detector = YOLOv8Detector(
        ...
        single_cls= rospy.get_param("~single_class") ## A list of class indices to consider. If None, all classes will be considered.
    )
